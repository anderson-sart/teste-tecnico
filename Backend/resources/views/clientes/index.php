                        ...
                        <table class="table table-hover align-middle">
                            <thead class="table-light">
                                <tr>
                                    ...
                                    <th style="cursor:pointer" @click="sortBy('nome')">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="btn-group btn-group-sm">
                                                <button class="btn btn-info" @click="view(c)" title="Ver">
                                                    <i class="bi bi-eye"></i>
                                                </button>
                                                <a :href="`/clientes/edit/${c.codigo}?page=${currentPage}`" class="btn btn-warning" title="Editar">
                                                    <i class="bi bi-pencil"></i>
                                                </a>
                                                <button class="btn btn-danger" @click="del(c)" title="Deletar">
...
function clientesPage() {
    return {
        ...
        toggleAll(checked) {
            this.selected = checked ? this.paginatedData.map(c => c.codigo) : [];
        },
        
        view(c) {
            const modal = document.querySelector('#viewModal [x-data]');
            Alpine.$data(modal).cliente = c;
            new bootstrap.Modal(document.getElementById('viewModal')).show();
        },
        
        del(c) {
            confirmDelete(
                `<p>Deseja realmente excluir o cliente?</p><p class="fw-bold mb-0">${c.nome}</p>`,
                async () => {
                    this.$store.loading.show();
                    try {
                        ...
                        this.$store.toast.show('Erro ao excluir cliente', 'error');
                        this.$store.loading.hide();
                    }
                }
            );
        ...
        deleteSelected() {
            confirmDelete(
                `<p>Deseja realmente excluir <strong>${this.selected.length}</strong> cliente(s)?</p>`,
                async () => {
                    this.$store.loading.show();
                    try {
                        ...
                        this.$store.toast.show('Erro ao excluir clientes', 'error');
                        this.$store.loading.hide();
                    }
                }
            );
