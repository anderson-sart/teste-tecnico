                    ...
                    <form @submit.prevent="save()">
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                <div class="char-counter" x-text="`${form.nome.length}/60`" 
                                     :class="{'text-danger': form.nome.length >= 54}"></div>
                            </div>
                            <div class="col-12">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" x-model="form.documento" 
                                       @input="maskDocument()" maxlength="18" required>
                            </div>
                            <div class="col-12">
                                ...
                                <div class="char-counter" x-text="`${form.endereco.length}/255`" 
                                     :class="{'text-danger': form.endereco.length >= 240}"></div>
                            </div>
                            <div class="col-12">
...
function clienteForm() {
    return {
        ...
        async init() {
            ...
            const match = path.match(/\/clientes\/edit\/(\d+)/);
            if (match) {
                this.id = match[1];
                await this.load();
            }
        ...
        async save() {
            this.$store.loading.show();
            try {
                const url = this.id ? '/api/clientes/' + this.id : '/api/clientes';
                ...
                if (res.ok) {
                    ...
                    if (data.errors) {
                        ...
                        this.$store.toast.show(data.message || 'Erro ao salvar cliente', 'error');
                    }
                    this.$store.loading.hide();
                }
            } catch (e) {
...
<?php $scripts = ob_get_clean(); $title = 'Formulário de Cliente'; include __DIR__ . '/../layout.php'; ?>
</div>
